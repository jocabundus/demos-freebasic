...
declare sub transformObject overload(o as ObjectType, polys() as PolyType, callback as sub(byref p as PolyType, v0 as VectorType, v1 as VectorType, v2 as VectorType), v0 as VectorType, v1 as VectorType, v2 as VectorType)
...
declare sub orientPoly(byref p as PolyType, up as VectorType, forward as VectorType, starboard as VectorType)
...
dim as VectorType up, forward, starboard
dim as PolyType p
dim as ObjectType o
...
objects( ObjectIds.Aeroplane ).vpos.y = -1
v.w = 0: v.x = 0: v.y = 0: v.z = 0
cloneVector objects( ObjectIds.Aeroplane ).vori, v
...
dim as VectorType vectors(2)
up.w        = 0: up.x        = 0: up.y        = 1: up.z        = 0
forward.w   = 0: forward.x   = 0: forward.y   = 0: forward.z   = 1
starboard.w = 0: starboard.x = 1: starboard.y = 0: starboard.z = 0
...
do
    ...
    transformObject o, polys(), @orientPoly, up, forward, starboard
    transformObject o, polys(), @panPoly, o.vpos
    ...
    if multikey(SC_UP) then
        starboard.w += 1: q = starboard: starboard.w =  1
        rotateVector up, q: unitVector up
        rotateVector forward, q: unitVector forward
    end if
    ...
    if multikey(SC_LSHIFT) then
        if multikey(SC_LEFT) then
            up.w += 1: q = up: up.w =  1
            rotateVector forward, q: unitVector forward
            rotateVector starboard, q: unitVector starboard
        end if
        ...
        if multikey(SC_LEFT) then
            forward.w += 1: q = forward: forward.w =  1
            rotateVector up, q: unitVector up
            rotateVector starboard, q: unitVector starboard
        end if
        if multikey(SC_RIGHT) then
...
sub orientPoly(byref p as PolyType, up as VectorType, forward as VectorType, starboard as VectorType)
    
    dim as VectorType u, v, w
    dim as integer n
    
    for n = 0 to 2
        u = up       : mulVector u, p.v(n).y
        v = forward  : mulVector v, p.v(n).z
        w = starboard: mulVector w, p.v(n).x
        p.v(n) = u
        addVectors p.v(n), v
        addVectors p.v(n), w
    next n
...
sub transformObject(o as ObjectType, polys() as PolyType, callback as sub(byref p as PolyType, v0 as VectorType, v1 as VectorType, v2 as VectorType), v0 as VectorType, v1 as VectorType, v2 as VectorType)
...
sub cloneObject(o as ObjectType, src as ObjectType, polys() as PolyType)
    ...
    cloneVector o.vpos, src.vpos
    cloneVector o.vori, src.vori
